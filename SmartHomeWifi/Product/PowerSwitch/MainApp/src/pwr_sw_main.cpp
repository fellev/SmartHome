...
/***************************************************************************/
/* WEB */
#define D_PWR_SW_MAIN_WEB_FORM_INPUT_NUM                			11
#define D_PWR_SW_MAIN_WEB_HTML_HEADER                               "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n<!DOCTYPE HTML>\r\n<html>"
#define D_PWR_SW_MAIN_WEB_CLIENT_CONNECTION_TIMEOUT                 20000 //ms

/* Buttons */
#define D_PWR_SW_MAIN_BTNCOUNT                                      1
#define D_PWR_SW_MAIN_BUTTON_PRESSED                                0
#define D_PWR_SW_MAIN_BUTTON_RELEASED                               1
#define D_PWR_SW_MAIN_BUTTON_BOUNCE_MS                              200  //timespan before another button change can occur
#define D_PWR_SW_MAIN_BUTTON_HOLD_TIME								1000 //ms

/* MQTT */
#define D_PWR_SW_MAIN_MQTT_TOPIC_PREFIX	                            "/CONTROLLERS/" PRODUCT_NAME
#define D_PWR_SW_MAIN_MQTT_TOPIC_IN                                 D_PWR_SW_MAIN_MQTT_TOPIC_PREFIX "/XXX/H2D"
#define D_PWR_SW_MAIN_MQTT_TOPIC_OUT                                D_PWR_SW_MAIN_MQTT_TOPIC_PREFIX "/XXX/D2H/"
#define D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_POWER                     D_PWR_SW_MAIN_MQTT_TOPIC_OUT "PWR"
#define D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_TIMER                     D_PWR_SW_MAIN_MQTT_TOPIC_OUT "TMR"
#define D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_STATUS                    D_PWR_SW_MAIN_MQTT_TOPIC_OUT "STATUS"
#define D_PWR_SW_MAIN_MQTT_TOPIC_MAX_SIZE							70
#define D_PWR_SW_MAIN_MQTT_TOPIC_NODE_ID_OFFSET						sizeof(D_PWR_SW_MAIN_MQTT_TOPIC_PREFIX)
#define D_PWR_SW_MAIN_MQTT_TOPIC_NODE_ID_SIZE                       3
#define D_PWR_SW_MAIN_MQTT_CONNECTION_RETRY_TIME                    60000 //ms
#define D_PWR_SW_MAIN_MQTT_STATUS_SEND_PWR_BIT                      0
#define D_PWR_SW_MAIN_MQTT_STATUS_SEND_TIMER_BIT                    1

/* Timers */
#define D_PWR_SW_MAIN_SEND_STATUS_RATE                              (1000*60)
#define D_PWR_SW_MAIN_UNALLOCATED_TIMER                             SimpleTimer::MAX_TIMERS

/* Leds */
#define D_PWR_SW_MAIN_LED_PULSE_PERIOD                              5000   //5s
#if defined(D_PWR_SW_MAIN_LED_ACTIVE_MODE_HIGH)
#define D_PWR_SW_MAIN_LED_ON_VALUE                                  PWMRANGE
#define D_PWR_SW_MAIN_LED_OFF_VALUE                                 0
#else
#define D_PWR_SW_MAIN_LED_ON_VALUE                                  0
#define D_PWR_SW_MAIN_LED_OFF_VALUE                                 PWMRANGE
#endif

#define D_PWR_SW_MAIN_POWER_ON_TIME_DEFAULT                         30 //min
...
typedef enum
{
   D_PWR_SW_MAIN_WEB_TYPE_CONFIGURATION,
   D_PWR_SW_MAIN_WEB_TYPE_TEST
}E_PWR_SW_MAIN_WEB_TYPE;

typedef enum
{
	D_PWR_SW_MAIN_POWER_CONTROL_OFF,
	D_PWR_SW_MAIN_POWER_CONTROL_ON
}E_PWR_SW_MAIN_POWER_CONTROL_MODE;
...
void f_whMainLaunchWeb(E_PWR_SW_MAIN_WEB_TYPE webtype, String param = String());
void f_whMainStopWeb();
int f_whLoadWebPage(E_PWR_SW_MAIN_WEB_TYPE webtype, String param);
...
byte btnLastState[]={D_PWR_SW_MAIN_BUTTON_RELEASED};
unsigned long btnLastPress[]={0,0};
byte btnState=D_PWR_SW_MAIN_BUTTON_RELEASED;
...
SimpleTimer timer;
int timerIdPower=D_PWR_SW_MAIN_UNALLOCATED_TIMER, timerIdStatus=D_PWR_SW_MAIN_UNALLOCATED_TIMER;

/* Power state*/
byte pwrState = D_PWR_SW_MAIN_POWER_CONTROL_OFF;
byte pwrStateOld = D_PWR_SW_MAIN_POWER_CONTROL_OFF;

/* MQTT */
char mqttTopicBuff[D_PWR_SW_MAIN_MQTT_TOPIC_MAX_SIZE];
char mqttNodeIdStr[D_PWR_SW_MAIN_MQTT_TOPIC_NODE_ID_SIZE+1] = { '0','0','0',0 };
...
void f_whMainGpioInit(void) {
	...
	pinMode(D_HW_CONFIG_GPIO_PIN_LED_POWER_STATUS, OUTPUT);   analogWrite(D_HW_CONFIG_GPIO_PIN_LED_POWER_STATUS, D_PWR_SW_MAIN_LED_OFF_VALUE);
}

void f_systemBootWifiConnected_whMain(void) {
//	f_whMainLaunchWeb(D_PWR_SW_MAIN_WEB_TYPE_TEST);
}

void f_SystemBootLaunchApServer_whMain(String wifiInRange) {
	f_whMainLaunchWeb(D_PWR_SW_MAIN_WEB_TYPE_CONFIGURATION, wifiInRange);
}


void f_whMainLaunchWeb(E_PWR_SW_MAIN_WEB_TYPE webtype, String param) {
...
//void f_whMainLaunchWeb(E_PWR_SW_MAIN_WEB_TYPE webtype ) {
...
int f_whLoadWebPage(E_PWR_SW_MAIN_WEB_TYPE webtype, String param) {
	...
	case 1:
		...
		if (client.connected() && !client.available()) {
			if (millis() - webClientConnectionTime > D_PWR_SW_MAIN_WEB_CLIENT_CONNECTION_TIMEOUT) {
				webPageST = 0;
				client.stop();
			}
			return (21);
	...
	case 2:
	{
		...
		client.flush();
		String s;
		if (webtype == D_PWR_SW_MAIN_WEB_TYPE_CONFIGURATION) {
			if (req == "/") {
				IPAddress ip = WiFi.softAPIP();
				String ipStr = String(ip[0]) + '.' + String(ip[1]) + '.'
						+ String(ip[2]) + '.' + String(ip[3]);
				s = D_PWR_SW_MAIN_WEB_HTML_HEADER PRODUCT_NAME " IP:";
				s += ipStr;
				s += "<p>";
				s += (String) param;
				...
				String inputs[D_PWR_SW_MAIN_WEB_FORM_INPUT_NUM];
				String t = req.substring(req.indexOf('=') + 1);
				for (int i = 0; i < D_PWR_SW_MAIN_WEB_FORM_INPUT_NUM; i++) {
				...
				gProductConfig_p->timer_manual_time = D_PWR_SW_MAIN_POWER_ON_TIME_DEFAULT;

				EEPROM_writeAnything(EEPROM_ADDR_CONFIGURATION, *gProductConfig_p);

				EEPROM.commit();

				s = D_PWR_SW_MAIN_WEB_HTML_HEADER PRODUCT_NAME;
				s += "Found ";
				s += req;
				s +=
...
void f_whMainPowerTimerTimeout() {
	f_whMainPowerControl((byte)D_PWR_SW_MAIN_POWER_CONTROL_OFF);
}

void f_whMainPowerTimerStatusSetBit() {
	D_UTILS_MASK_SET_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_TIMER_BIT);
}
...
void f_whMainPowerTimerStatusSend() {
	...
	client.publish(f_whMainMqttGetTopic(D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_TIMER), msgBuf);
...
void f_whMainMqttCallback(char* topic, byte* payload, unsigned int length) {
	...
	if (length == 2 && payload[0] == 'O' && payload[1] == 'N') { /* ON */
		...
		f_whMainPowerControl((byte) D_PWR_SW_MAIN_POWER_CONTROL_ON);
		...
		int time = atol(buf);
		f_whMainPowerControl((byte) D_PWR_SW_MAIN_POWER_CONTROL_ON, time);
...
//#if defined(DEBUG)
		...
		DEBUG_PRINTLN("clearing eeprom");
		client.publish(
				f_whMainMqttGetTopic(D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_STATUS),
				"clearing eeprom");
		delay(1000);
		memset(gProductConfig_p, 0, sizeof(*gProductConfig_p));
		EEPROM_writeAnything(EEPROM_ADDR_CONFIGURATION, *gProductConfig_p);
...
char * f_whMainMqttGetTopic(const char* topicTemplate) {
	...
	memcpy(&mqttTopicBuff[D_PWR_SW_MAIN_MQTT_TOPIC_NODE_ID_OFFSET], mqttNodeIdStr, D_PWR_SW_MAIN_MQTT_TOPIC_NODE_ID_SIZE);
...
void f_whMainMqttReconnect() {
	...
	if (!mqttIsNotFirstConnection || ((now-mqttLastReconnectionTry) > D_PWR_SW_MAIN_MQTT_CONNECTION_RETRY_TIME)) {
		...
		if (client.connect(mqttTopicBuff)) {
		  ...
		  WiFi.mode(WIFI_STA);
		  f_whMainStopWeb();
		  client.publish(f_whMainMqttGetTopic(D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_STATUS),"CONNECTED");
		  client.subscribe(f_whMainMqttGetTopic(D_PWR_SW_MAIN_MQTT_TOPIC_IN));
		  D_UTILS_MASK_SET_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_PWR_BIT);
		  D_UTILS_MASK_SET_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_TIMER_BIT);
...
void f_whMainPowerControl(byte mode, int offTimeMin, bool sendStatus) {
	...
	timerIdPower = timerIdStatus = D_PWR_SW_MAIN_UNALLOCATED_TIMER;

	if (mode == (uint8_t)D_PWR_SW_MAIN_POWER_CONTROL_OFF)
	{
	...
	else if (mode == (uint8_t)D_PWR_SW_MAIN_POWER_CONTROL_ON)
	{
		...
		if (offTimeMin > 0) {
			...
			timerIdStatus = timer.setInterval(D_PWR_SW_MAIN_SEND_STATUS_RATE, f_whMainPowerTimerStatusSetBit);
			if (offTimeMin > 0)
...
void f_whMainHandleButtons() {
	  btnIndex++;
	  if (btnIndex>D_PWR_SW_MAIN_BTNCOUNT-1) btnIndex=0;

	  btnState = digitalRead(btn[btnIndex]);

	  if (btnState != btnLastState[btnIndex] && now-btnLastPress[btnIndex] >= D_PWR_SW_MAIN_BUTTON_BOUNCE_MS) //button event happened
	  {
	    DEBUG_PRINT("Button ");DEBUG_PRINTDEC(btnIndex);DEBUG_PRINTLN((btnState == D_PWR_SW_MAIN_BUTTON_PRESSED) ? " pressed" : " released");
	    btnLastState[btnIndex] = btnState;
	    btnLastPress[btnIndex] = now;
	    if (btnState == D_PWR_SW_MAIN_BUTTON_PRESSED)
	    {
		  f_whMainPowerControl(pwrState == D_PWR_SW_MAIN_POWER_CONTROL_OFF ? D_PWR_SW_MAIN_POWER_CONTROL_ON : D_PWR_SW_MAIN_POWER_CONTROL_OFF, gProductConfig_p->timer_manual_time);
	    }
	    if (btnHoldEnabled) {
	  ...
	  if (!btnHoldEnabled && (btnState == D_PWR_SW_MAIN_BUTTON_PRESSED) && (now-btnLastPress[btnIndex] >= D_PWR_SW_MAIN_BUTTON_HOLD_TIME))
	  {
		  btnHoldEnabled = true;
		  if (pwrState == D_PWR_SW_MAIN_POWER_CONTROL_ON)
			  f_whMainPowerControl((byte)D_PWR_SW_MAIN_POWER_CONTROL_ON, 0);
	  }
}
...
inline void f_whMainLedPulseStart( byte numOfPulses ) {
    ...
    ledPulseValue = -(D_PWR_SW_MAIN_LED_PULSE_PERIOD/256);
#else
    ledPulseDirection=true;
    ledPulseValue = PWMRANGE+(D_PWR_SW_MAIN_LED_PULSE_PERIOD/256);
#endif
    ledBlinkCount = 0;
...
bool f_whMainHandleLeds() {
	...
	if (ledPulseEnabled && (ledBlinksRequestedNum > ledBlinkCount)) {
	    if (millis()-(ledPulseTimestamp) > D_PWR_SW_MAIN_LED_PULSE_PERIOD/350)
	    {
	      ledPulseValue = ledPulseDirection ? ledPulseValue + D_PWR_SW_MAIN_LED_PULSE_PERIOD/256 : ledPulseValue - D_PWR_SW_MAIN_LED_PULSE_PERIOD/256;
...
//	      DEBUG_PRINT("LED:");DEBUG_PRINTLN(ledPulseValue, DEC);
		...
		if (isLedsNeedUpdate) {
			if (pwrState == D_PWR_SW_MAIN_POWER_CONTROL_ON) {
				analogWrite(D_HW_CONFIG_GPIO_PIN_LED_POWER_STATUS, D_PWR_SW_MAIN_LED_ON_VALUE);
			} else {
				analogWrite(D_HW_CONFIG_GPIO_PIN_LED_POWER_STATUS, D_PWR_SW_MAIN_LED_OFF_VALUE);
			}
		}
	}
...
void f_whMainHandleMqttStatus() {
	...
	if (D_UTILS_MASK_GET_BIT(mqttStatusSendMask, D_PWR_SW_MAIN_MQTT_STATUS_SEND_PWR_BIT)) {
		D_UTILS_MASK_CLR_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_PWR_BIT);
		client.publish(f_whMainMqttGetTopic(D_PWR_SW_MAIN_MQTT_TOPIC_TEMPLATE_POWER),pwrState == D_PWR_SW_MAIN_POWER_CONTROL_OFF ? "OFF" : "ON");
	} else if (D_UTILS_MASK_GET_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_TIMER_BIT)) {
		D_UTILS_MASK_CLR_BIT(mqttStatusSendMask,D_PWR_SW_MAIN_MQTT_STATUS_SEND_TIMER_BIT);
...
void loop() {
	...
	if (!isBusy) {
		f_whLoadWebPage(D_PWR_SW_MAIN_WEB_TYPE_CONFIGURATION, configServerParam);
		if (WiFi.isConnected()){
			if (!client.connected()) {
